...
def finish_exam():
    ...
    st.write(f"- Catégorie A : {len([r for r in st.session_state['responses_a'] if r['is_correct']])} bonnes réponses sur 33")
    st.write(f"- Catégorie C : {len([r for r in st.session_state['responses_c'] if r['is_correct']])} bonnes réponses sur 87")
